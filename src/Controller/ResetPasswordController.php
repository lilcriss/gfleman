...
class ResetPasswordController extends AbstractController
{
    ...
    public function index(Request $request): Response
    {
        ...
        if ($request->get('email')) {
            ...
            if ($user) {
                ...
                // 2 : Envoyer mail à l'utilisateur avec lien pour mettre à jour son mot de passe;

                $url = $this->generateUrl('update_password', [
                    'token' => $reset_password->getToken()
                ]);

                $content = 'Bonjour, '.$user->getFirstname().'<br/>Vous avez demandé à réinitialiser votre mot de passe sur Afrobio.<br/><br/>';
                $content .= 'Merci de bien vouloir cliquer sur le lien suivant <a href="'.$url.'"> pour mettre à jour votre mot de passe</a>.';

                $mail = new Mail();
                $mail->send($user->getEmail(), $user->getFirstname().''.$user->getLastname(), 'Réinitialisez votre mot de passe sur Afrobio.', $content);
    ...
    public function update(Request $request, $token, UserPasswordEncoderInterface $encoder): Response
    {
        ...
        {
            $this->addFlash('notice', 'Votre demande de mot de passe a expiré. Merci de bien vouloir la renouveler.');
            return $this->redirectToRoute('reset_password');
        }
